#
# This process the LLVM unittest for LoopInfoTest.cpp to get the IR code it contains as examples.
#
...
bounds = lapply(mods, function(m) { la = loopAnalysis(m[[ names(m)[1] ]]) ; loops = getLoops(la); getBounds(loops[[1]], la$scalar)})
