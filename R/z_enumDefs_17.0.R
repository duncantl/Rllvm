...
"CStringKind", "StdStringKind", "PtrAndLengthKind", "FormatvObjectKind", 
...
Memory = 80L, NoFPClass = 81L, StackAlignment = 82L, UWTable = 83L, 
VScaleRange = 84L, LastIntAttr = 84L, EndAttrKinds = 85L, EmptyKey = 86L, 
TombstoneKey = 87L)
...
`not_intrinsic` = 0L
`abs` = 1L
`addressofreturnaddress` = 2L
`adjust_trampoline` = 3L
`annotation` = 4L
`arithmetic_fence` = 5L
`asan_check_memaccess` = 6L
`assume` = 7L
`bitreverse` = 8L
`bswap` = 9L
`call_preallocated_arg` = 10L
`call_preallocated_setup` = 11L
`call_preallocated_teardown` = 12L
`callbr_landingpad` = 13L
`canonicalize` = 14L
`ceil` = 15L
`clear_cache` = 16L
`codeview_annotation` = 17L
`convert_from_fp16` = 18L
`convert_to_fp16` = 19L
`copysign` = 20L
`coro_align` = 21L
`coro_alloc` = 22L
`coro_alloca_alloc` = 23L
`coro_alloca_free` = 24L
`coro_alloca_get` = 25L
`coro_async_context_alloc` = 26L
`coro_async_context_dealloc` = 27L
`coro_async_resume` = 28L
`coro_async_size_replace` = 29L
`coro_begin` = 30L
`coro_destroy` = 31L
`coro_done` = 32L
`coro_end` = 33L
...
`coro_id_async` = 38L
`coro_id_retcon` = 39L
`coro_id_retcon_once` = 40L
`coro_noop` = 41L
`coro_prepare_async` = 42L
`coro_prepare_retcon` = 43L
`coro_promise` = 44L
`coro_resume` = 45L
`coro_save` = 46L
`coro_size` = 47L
`coro_subfn_addr` = 48L
`coro_suspend` = 49L
`coro_suspend_async` = 50L
`coro_suspend_retcon` = 51L
`cos` = 52L
`ctlz` = 53L
`ctpop` = 54L
`cttz` = 55L
...
`donothing` = 61L
`eh_dwarf_cfa` = 62L
`eh_exceptioncode` = 63L
`eh_exceptionpointer` = 64L
`eh_recoverfp` = 65L
`eh_return_i32` = 66L
`eh_return_i64` = 67L
`eh_sjlj_callsite` = 68L
`eh_sjlj_functioncontext` = 69L
`eh_sjlj_longjmp` = 70L
`eh_sjlj_lsda` = 71L
`eh_sjlj_setjmp` = 72L
`eh_sjlj_setup_dispatch` = 73L
`eh_typeid_for` = 74L
`eh_unwind_init` = 75L
`exp` = 76L
`exp2` = 77L
`expect` = 78L
`expect_with_probability` = 79L
`experimental_constrained_ceil` = 80L
`experimental_constrained_cos` = 81L
`experimental_constrained_exp` = 82L
`experimental_constrained_exp2` = 83L
`experimental_constrained_fadd` = 84L
`experimental_constrained_fcmp` = 85L
`experimental_constrained_fcmps` = 86L
`experimental_constrained_fdiv` = 87L
`experimental_constrained_floor` = 88L
`experimental_constrained_fma` = 89L
`experimental_constrained_fmul` = 90L
`experimental_constrained_fmuladd` = 91L
`experimental_constrained_fpext` = 92L
`experimental_constrained_fptosi` = 93L
`experimental_constrained_fptoui` = 94L
`experimental_constrained_fptrunc` = 95L
`experimental_constrained_frem` = 96L
`experimental_constrained_fsub` = 97L
`experimental_constrained_ldexp` = 98L
`experimental_constrained_llrint` = 99L
`experimental_constrained_llround` = 100L
`experimental_constrained_log` = 101L
`experimental_constrained_log10` = 102L
`experimental_constrained_log2` = 103L
`experimental_constrained_lrint` = 104L
`experimental_constrained_lround` = 105L
`experimental_constrained_maximum` = 106L
`experimental_constrained_maxnum` = 107L
`experimental_constrained_minimum` = 108L
`experimental_constrained_minnum` = 109L
`experimental_constrained_nearbyint` = 110L
`experimental_constrained_pow` = 111L
`experimental_constrained_powi` = 112L
`experimental_constrained_rint` = 113L
`experimental_constrained_round` = 114L
`experimental_constrained_roundeven` = 115L
`experimental_constrained_sin` = 116L
`experimental_constrained_sitofp` = 117L
`experimental_constrained_sqrt` = 118L
`experimental_constrained_trunc` = 119L
`experimental_constrained_uitofp` = 120L
`experimental_convergence_anchor` = 121L
`experimental_convergence_entry` = 122L
`experimental_convergence_loop` = 123L
`experimental_deoptimize` = 124L
`experimental_gc_get_pointer_base` = 125L
`experimental_gc_get_pointer_offset` = 126L
`experimental_gc_relocate` = 127L
`experimental_gc_result` = 128L
`experimental_gc_statepoint` = 129L
`experimental_get_vector_length` = 130L
`experimental_guard` = 131L
`experimental_noalias_scope_decl` = 132L
`experimental_patchpoint_i64` = 133L
`experimental_patchpoint_void` = 134L
`experimental_stackmap` = 135L
`experimental_stepvector` = 136L
`experimental_vector_deinterleave2` = 137L
`experimental_vector_interleave2` = 138L
`experimental_vector_reverse` = 139L
`experimental_vector_splice` = 140L
`experimental_vp_splice` = 141L
`experimental_vp_strided_load` = 142L
`experimental_vp_strided_store` = 143L
`experimental_widenable_condition` = 144L
`fabs` = 145L
`floor` = 146L
`fma` = 147L
`fmuladd` = 148L
...
`gcroot` = 157L
`gcwrite` = 158L
`get_active_lane_mask` = 159L
`get_dynamic_area_offset` = 160L
`get_fpenv` = 161L
`get_rounding` = 162L
`hwasan_check_memaccess` = 163L
`hwasan_check_memaccess_shortgranules` = 164L
`icall_branch_funnel` = 165L
`init_trampoline` = 166L
`instrprof_cover` = 167L
`instrprof_increment` = 168L
`instrprof_increment_step` = 169L
`instrprof_timestamp` = 170L
`instrprof_value_profile` = 171L
`invariant_end` = 172L
`invariant_start` = 173L
`is_constant` = 174L
`is_fpclass` = 175L
`launder_invariant_group` = 176L
`ldexp` = 177L
`lifetime_end` = 178L
`lifetime_start` = 179L
`llrint` = 180L
...
`log2` = 188L
`loop_decrement` = 189L
`loop_decrement_reg` = 190L
`lrint` = 191L
`lround` = 192L
`masked_compressstore` = 193L
`masked_expandload` = 194L
`masked_gather` = 195L
`masked_load` = 196L
`masked_scatter` = 197L
`masked_store` = 198L
`matrix_column_major_load` = 199L
`matrix_column_major_store` = 200L
`matrix_multiply` = 201L
`matrix_transpose` = 202L
`maximum` = 203L
`maxnum` = 204L
`memcpy` = 205L
`memcpy_element_unordered_atomic` = 206L
`memcpy_inline` = 207L
`memmove` = 208L
`memmove_element_unordered_atomic` = 209L
`memset` = 210L
`memset_element_unordered_atomic` = 211L
`memset_inline` = 212L
`minimum` = 213L
`minnum` = 214L
`nearbyint` = 215L
`objc_arc_annotation_bottomup_bbend` = 216L
`objc_arc_annotation_bottomup_bbstart` = 217L
`objc_arc_annotation_topdown_bbend` = 218L
`objc_arc_annotation_topdown_bbstart` = 219L
`objc_autorelease` = 220L
`objc_autoreleasePoolPop` = 221L
`objc_autoreleasePoolPush` = 222L
`objc_autoreleaseReturnValue` = 223L
`objc_clang_arc_noop_use` = 224L
`objc_clang_arc_use` = 225L
`objc_copyWeak` = 226L
`objc_destroyWeak` = 227L
`objc_initWeak` = 228L
`objc_loadWeak` = 229L
`objc_loadWeakRetained` = 230L
`objc_moveWeak` = 231L
`objc_release` = 232L
`objc_retain` = 233L
`objc_retain_autorelease` = 234L
`objc_retainAutorelease` = 235L
`objc_retainAutoreleaseReturnValue` = 236L
`objc_retainAutoreleasedReturnValue` = 237L
`objc_retainBlock` = 238L
`objc_retainedObject` = 239L
`objc_storeStrong` = 240L
`objc_storeWeak` = 241L
`objc_sync_enter` = 242L
`objc_sync_exit` = 243L
`objc_unretainedObject` = 244L
`objc_unretainedPointer` = 245L
`objc_unsafeClaimAutoreleasedReturnValue` = 246L
`objectsize` = 247L
`pcmarker` = 248L
`pow` = 249L
`powi` = 250L
`prefetch` = 251L
`preserve_array_access_index` = 252L
`preserve_struct_access_index` = 253L
`preserve_union_access_index` = 254L
`pseudoprobe` = 255L
`ptr_annotation` = 256L
`ptrauth_auth` = 257L
`ptrauth_blend` = 258L
`ptrauth_resign` = 259L
`ptrauth_sign` = 260L
`ptrauth_sign_generic` = 261L
`ptrauth_strip` = 262L
`ptrmask` = 263L
`public_type_test` = 264L
`read_register` = 265L
`read_volatile_register` = 266L
`readcyclecounter` = 267L
`reset_fpenv` = 268L
`returnaddress` = 269L
`rint` = 270L
`round` = 271L
`roundeven` = 272L
`sadd_sat` = 273L
`sadd_with_overflow` = 274L
`sdiv_fix` = 275L
`sdiv_fix_sat` = 276L
`seh_scope_begin` = 277L
`seh_scope_end` = 278L
`seh_try_begin` = 279L
`seh_try_end` = 280L
`set_fpenv` = 281L
`set_loop_iterations` = 282L
`set_rounding` = 283L
`sideeffect` = 284L
`sin` = 285L
`smax` = 286L
...
`smul_fix` = 288L
`smul_fix_sat` = 289L
`smul_with_overflow` = 290L
`sponentry` = 291L
`sqrt` = 292L
`ssa_copy` = 293L
`sshl_sat` = 294L
`ssub_sat` = 295L
`ssub_with_overflow` = 296L
`stackguard` = 297L
`stackprotector` = 298L
`stackrestore` = 299L
`stacksave` = 300L
`start_loop_iterations` = 301L
`strip_invariant_group` = 302L
`swift_async_context_addr` = 303L
`test_set_loop_iterations` = 304L
`test_start_loop_iterations` = 305L
`thread_pointer` = 306L
`threadlocal_address` = 307L
`trap` = 308L
`trunc` = 309L
`type_checked_load` = 310L
`type_checked_load_relative` = 311L
`type_test` = 312L
`uadd_sat` = 313L
`uadd_with_overflow` = 314L
`ubsantrap` = 315L
`udiv_fix` = 316L
`udiv_fix_sat` = 317L
`umax` = 318L
...
`umul_fix` = 320L
`umul_fix_sat` = 321L
`umul_with_overflow` = 322L
`ushl_sat` = 323L
`usub_sat` = 324L
`usub_with_overflow` = 325L
`vacopy` = 326L
`vaend` = 327L
`vastart` = 328L
`var_annotation` = 329L
`vector_extract` = 330L
`vector_insert` = 331L
`vector_reduce_add` = 332L
`vector_reduce_and` = 333L
`vector_reduce_fadd` = 334L
`vector_reduce_fmax` = 335L
`vector_reduce_fmaximum` = 336L
`vector_reduce_fmin` = 337L
`vector_reduce_fminimum` = 338L
`vector_reduce_fmul` = 339L
`vector_reduce_mul` = 340L
`vector_reduce_or` = 341L
`vector_reduce_smax` = 342L
`vector_reduce_smin` = 343L
`vector_reduce_umax` = 344L
`vector_reduce_umin` = 345L
`vector_reduce_xor` = 346L
`vp_abs` = 347L
`vp_add` = 348L
`vp_and` = 349L
`vp_ashr` = 350L
...
`vscale` = 424L
`write_register` = 425L
`xray_customevent` = 426L
`xray_typedevent` = 427L
`num_intrinsics` = 11926L



 `IndependentIntrinsics`  = c(not_intrinsic = 0L, abs = 1L, addressofreturnaddress = 2L, 
adjust_trampoline = 3L, annotation = 4L, arithmetic_fence = 5L, 
asan_check_memaccess = 6L, assume = 7L, bitreverse = 8L, bswap = 9L, 
call_preallocated_arg = 10L, call_preallocated_setup = 11L, call_preallocated_teardown = 12L, 
callbr_landingpad = 13L, canonicalize = 14L, ceil = 15L, clear_cache = 16L, 
codeview_annotation = 17L, convert_from_fp16 = 18L, convert_to_fp16 = 19L, 
copysign = 20L, coro_align = 21L, coro_alloc = 22L, coro_alloca_alloc = 23L, 
coro_alloca_free = 24L, coro_alloca_get = 25L, coro_async_context_alloc = 26L, 
coro_async_context_dealloc = 27L, coro_async_resume = 28L, coro_async_size_replace = 29L, 
coro_begin = 30L, coro_destroy = 31L, coro_done = 32L, coro_end = 33L, 
coro_end_async = 34L, coro_frame = 35L, coro_free = 36L, coro_id = 37L, 
coro_id_async = 38L, coro_id_retcon = 39L, coro_id_retcon_once = 40L, 
coro_noop = 41L, coro_prepare_async = 42L, coro_prepare_retcon = 43L, 
coro_promise = 44L, coro_resume = 45L, coro_save = 46L, coro_size = 47L, 
coro_subfn_addr = 48L, coro_suspend = 49L, coro_suspend_async = 50L, 
coro_suspend_retcon = 51L, cos = 52L, ctlz = 53L, ctpop = 54L, 
cttz = 55L, dbg_assign = 56L, dbg_declare = 57L, dbg_label = 58L, 
dbg_value = 59L, debugtrap = 60L, donothing = 61L, eh_dwarf_cfa = 62L, 
eh_exceptioncode = 63L, eh_exceptionpointer = 64L, eh_recoverfp = 65L, 
eh_return_i32 = 66L, eh_return_i64 = 67L, eh_sjlj_callsite = 68L, 
eh_sjlj_functioncontext = 69L, eh_sjlj_longjmp = 70L, eh_sjlj_lsda = 71L, 
eh_sjlj_setjmp = 72L, eh_sjlj_setup_dispatch = 73L, eh_typeid_for = 74L, 
eh_unwind_init = 75L, exp = 76L, exp2 = 77L, expect = 78L, expect_with_probability = 79L, 
experimental_constrained_ceil = 80L, experimental_constrained_cos = 81L, 
experimental_constrained_exp = 82L, experimental_constrained_exp2 = 83L, 
experimental_constrained_fadd = 84L, experimental_constrained_fcmp = 85L, 
experimental_constrained_fcmps = 86L, experimental_constrained_fdiv = 87L, 
experimental_constrained_floor = 88L, experimental_constrained_fma = 89L, 
experimental_constrained_fmul = 90L, experimental_constrained_fmuladd = 91L, 
experimental_constrained_fpext = 92L, experimental_constrained_fptosi = 93L, 
experimental_constrained_fptoui = 94L, experimental_constrained_fptrunc = 95L, 
experimental_constrained_frem = 96L, experimental_constrained_fsub = 97L, 
experimental_constrained_ldexp = 98L, experimental_constrained_llrint = 99L, 
experimental_constrained_llround = 100L, experimental_constrained_log = 101L, 
experimental_constrained_log10 = 102L, experimental_constrained_log2 = 103L, 
experimental_constrained_lrint = 104L, experimental_constrained_lround = 105L, 
experimental_constrained_maximum = 106L, experimental_constrained_maxnum = 107L, 
experimental_constrained_minimum = 108L, experimental_constrained_minnum = 109L, 
experimental_constrained_nearbyint = 110L, experimental_constrained_pow = 111L, 
experimental_constrained_powi = 112L, experimental_constrained_rint = 113L, 
experimental_constrained_round = 114L, experimental_constrained_roundeven = 115L, 
experimental_constrained_sin = 116L, experimental_constrained_sitofp = 117L, 
experimental_constrained_sqrt = 118L, experimental_constrained_trunc = 119L, 
experimental_constrained_uitofp = 120L, experimental_convergence_anchor = 121L, 
experimental_convergence_entry = 122L, experimental_convergence_loop = 123L, 
experimental_deoptimize = 124L, experimental_gc_get_pointer_base = 125L, 
experimental_gc_get_pointer_offset = 126L, experimental_gc_relocate = 127L, 
experimental_gc_result = 128L, experimental_gc_statepoint = 129L, 
experimental_get_vector_length = 130L, experimental_guard = 131L, 
experimental_noalias_scope_decl = 132L, experimental_patchpoint_i64 = 133L, 
experimental_patchpoint_void = 134L, experimental_stackmap = 135L, 
experimental_stepvector = 136L, experimental_vector_deinterleave2 = 137L, 
experimental_vector_interleave2 = 138L, experimental_vector_reverse = 139L, 
experimental_vector_splice = 140L, experimental_vp_splice = 141L, 
experimental_vp_strided_load = 142L, experimental_vp_strided_store = 143L, 
experimental_widenable_condition = 144L, fabs = 145L, floor = 146L, 
fma = 147L, fmuladd = 148L, fptosi_sat = 149L, fptoui_sat = 150L, 
fptrunc_round = 151L, frameaddress = 152L, frexp = 153L, fshl = 154L, 
fshr = 155L, gcread = 156L, gcroot = 157L, gcwrite = 158L, get_active_lane_mask = 159L, 
get_dynamic_area_offset = 160L, get_fpenv = 161L, get_rounding = 162L, 
hwasan_check_memaccess = 163L, hwasan_check_memaccess_shortgranules = 164L, 
icall_branch_funnel = 165L, init_trampoline = 166L, instrprof_cover = 167L, 
instrprof_increment = 168L, instrprof_increment_step = 169L, 
instrprof_timestamp = 170L, instrprof_value_profile = 171L, invariant_end = 172L, 
invariant_start = 173L, is_constant = 174L, is_fpclass = 175L, 
launder_invariant_group = 176L, ldexp = 177L, lifetime_end = 178L, 
lifetime_start = 179L, llrint = 180L, llround = 181L, load_relative = 182L, 
localaddress = 183L, localescape = 184L, localrecover = 185L, 
log = 186L, log10 = 187L, log2 = 188L, loop_decrement = 189L, 
loop_decrement_reg = 190L, lrint = 191L, lround = 192L, masked_compressstore = 193L, 
masked_expandload = 194L, masked_gather = 195L, masked_load = 196L, 
masked_scatter = 197L, masked_store = 198L, matrix_column_major_load = 199L, 
matrix_column_major_store = 200L, matrix_multiply = 201L, matrix_transpose = 202L, 
maximum = 203L, maxnum = 204L, memcpy = 205L, memcpy_element_unordered_atomic = 206L, 
memcpy_inline = 207L, memmove = 208L, memmove_element_unordered_atomic = 209L, 
memset = 210L, memset_element_unordered_atomic = 211L, memset_inline = 212L, 
minimum = 213L, minnum = 214L, nearbyint = 215L, objc_arc_annotation_bottomup_bbend = 216L, 
objc_arc_annotation_bottomup_bbstart = 217L, objc_arc_annotation_topdown_bbend = 218L, 
objc_arc_annotation_topdown_bbstart = 219L, objc_autorelease = 220L, 
objc_autoreleasePoolPop = 221L, objc_autoreleasePoolPush = 222L, 
objc_autoreleaseReturnValue = 223L, objc_clang_arc_noop_use = 224L, 
objc_clang_arc_use = 225L, objc_copyWeak = 226L, objc_destroyWeak = 227L, 
objc_initWeak = 228L, objc_loadWeak = 229L, objc_loadWeakRetained = 230L, 
objc_moveWeak = 231L, objc_release = 232L, objc_retain = 233L, 
objc_retain_autorelease = 234L, objc_retainAutorelease = 235L, 
objc_retainAutoreleaseReturnValue = 236L, objc_retainAutoreleasedReturnValue = 237L, 
objc_retainBlock = 238L, objc_retainedObject = 239L, objc_storeStrong = 240L, 
objc_storeWeak = 241L, objc_sync_enter = 242L, objc_sync_exit = 243L, 
objc_unretainedObject = 244L, objc_unretainedPointer = 245L, 
objc_unsafeClaimAutoreleasedReturnValue = 246L, objectsize = 247L, 
pcmarker = 248L, pow = 249L, powi = 250L, prefetch = 251L, preserve_array_access_index = 252L, 
preserve_struct_access_index = 253L, preserve_union_access_index = 254L, 
pseudoprobe = 255L, ptr_annotation = 256L, ptrauth_auth = 257L, 
ptrauth_blend = 258L, ptrauth_resign = 259L, ptrauth_sign = 260L, 
ptrauth_sign_generic = 261L, ptrauth_strip = 262L, ptrmask = 263L, 
public_type_test = 264L, read_register = 265L, read_volatile_register = 266L, 
readcyclecounter = 267L, reset_fpenv = 268L, returnaddress = 269L, 
rint = 270L, round = 271L, roundeven = 272L, sadd_sat = 273L, 
sadd_with_overflow = 274L, sdiv_fix = 275L, sdiv_fix_sat = 276L, 
seh_scope_begin = 277L, seh_scope_end = 278L, seh_try_begin = 279L, 
seh_try_end = 280L, set_fpenv = 281L, set_loop_iterations = 282L, 
set_rounding = 283L, sideeffect = 284L, sin = 285L, smax = 286L, 
smin = 287L, smul_fix = 288L, smul_fix_sat = 289L, smul_with_overflow = 290L, 
sponentry = 291L, sqrt = 292L, ssa_copy = 293L, sshl_sat = 294L, 
ssub_sat = 295L, ssub_with_overflow = 296L, stackguard = 297L, 
stackprotector = 298L, stackrestore = 299L, stacksave = 300L, 
start_loop_iterations = 301L, strip_invariant_group = 302L, swift_async_context_addr = 303L, 
test_set_loop_iterations = 304L, test_start_loop_iterations = 305L, 
thread_pointer = 306L, threadlocal_address = 307L, trap = 308L, 
trunc = 309L, type_checked_load = 310L, type_checked_load_relative = 311L, 
type_test = 312L, uadd_sat = 313L, uadd_with_overflow = 314L, 
ubsantrap = 315L, udiv_fix = 316L, udiv_fix_sat = 317L, umax = 318L, 
umin = 319L, umul_fix = 320L, umul_fix_sat = 321L, umul_with_overflow = 322L, 
ushl_sat = 323L, usub_sat = 324L, usub_with_overflow = 325L, 
vacopy = 326L, vaend = 327L, vastart = 328L, var_annotation = 329L, 
vector_extract = 330L, vector_insert = 331L, vector_reduce_add = 332L, 
vector_reduce_and = 333L, vector_reduce_fadd = 334L, vector_reduce_fmax = 335L, 
vector_reduce_fmaximum = 336L, vector_reduce_fmin = 337L, vector_reduce_fminimum = 338L, 
vector_reduce_fmul = 339L, vector_reduce_mul = 340L, vector_reduce_or = 341L, 
vector_reduce_smax = 342L, vector_reduce_smin = 343L, vector_reduce_umax = 344L, 
vector_reduce_umin = 345L, vector_reduce_xor = 346L, vp_abs = 347L, 
vp_add = 348L, vp_and = 349L, vp_ashr = 350L, vp_bitreverse = 351L, 
vp_bswap = 352L, vp_ceil = 353L, vp_copysign = 354L, vp_ctlz = 355L, 
vp_ctpop = 356L, vp_cttz = 357L, vp_fabs = 358L, vp_fadd = 359L, 
vp_fcmp = 360L, vp_fdiv = 361L, vp_floor = 362L, vp_fma = 363L, 
vp_fmul = 364L, vp_fmuladd = 365L, vp_fneg = 366L, vp_fpext = 367L, 
vp_fptosi = 368L, vp_fptoui = 369L, vp_fptrunc = 370L, vp_frem = 371L, 
vp_fshl = 372L, vp_fshr = 373L, vp_fsub = 374L, vp_gather = 375L, 
vp_icmp = 376L, vp_inttoptr = 377L, vp_load = 378L, vp_lshr = 379L, 
vp_maxnum = 380L, vp_merge = 381L, vp_minnum = 382L, vp_mul = 383L, 
vp_nearbyint = 384L, vp_or = 385L, vp_ptrtoint = 386L, vp_reduce_add = 387L, 
vp_reduce_and = 388L, vp_reduce_fadd = 389L, vp_reduce_fmax = 390L, 
vp_reduce_fmin = 391L, vp_reduce_fmul = 392L, vp_reduce_mul = 393L, 
vp_reduce_or = 394L, vp_reduce_smax = 395L, vp_reduce_smin = 396L, 
vp_reduce_umax = 397L, vp_reduce_umin = 398L, vp_reduce_xor = 399L, 
vp_rint = 400L, vp_round = 401L, vp_roundeven = 402L, vp_roundtozero = 403L, 
vp_scatter = 404L, vp_sdiv = 405L, vp_select = 406L, vp_sext = 407L, 
vp_shl = 408L, vp_sitofp = 409L, vp_smax = 410L, vp_smin = 411L, 
vp_sqrt = 412L, vp_srem = 413L, vp_store = 414L, vp_sub = 415L, 
vp_trunc = 416L, vp_udiv = 417L, vp_uitofp = 418L, vp_umax = 419L, 
vp_umin = 420L, vp_urem = 421L, vp_xor = 422L, vp_zext = 423L, 
vscale = 424L, write_register = 425L, xray_customevent = 426L, 
xray_typedevent = 427L, num_intrinsics = 11926L)
...
UEFI = 15L, Win32 = 16L, ZOS = 17L, Haiku = 18L, Minix = 19L, 
RTEMS = 20L, NaCl = 21L, AIX = 22L, CUDA = 23L, NVCL = 24L, AMDHSA = 25L, 
PS4 = 26L, PS5 = 27L, ELFIAMCU = 28L, TvOS = 29L, WatchOS = 30L, 
DriverKit = 31L, Mesa3D = 32L, Contiki = 33L, AMDPAL = 34L, HermitCore = 35L, 
Hurd = 36L, WASI = 37L, Emscripten = 38L, ShaderModel = 39L, 
LiteOS = 40L, LastOSType = 40L)
...
`AIX` = 6L
 ...
 `ExceptionHandling`  = structure(0:6, names = c("None", "DwarfCFI", "SjLj", "ARM", "WinEH", 
"Wasm", "AIX"))
...
`NoAlloc` = 2L
 ...
 `MemLifetimePolicy`  = structure(0:2, names = c("Standard", "Finalize", "NoAlloc"))
...
`goff_object` = 8L
`macho_object` = 9L
`macho_executable` = 10L
`macho_fixed_virtual_memory_shared_lib` = 11L
`macho_core` = 12L
`macho_preload_executable` = 13L
`macho_dynamically_linked_shared_lib` = 14L
`macho_dynamic_linker` = 15L
`macho_bundle` = 16L
`macho_dynamically_linked_shared_lib_stub` = 17L
`macho_dsym_companion` = 18L
`macho_kext_bundle` = 19L
`macho_universal_binary` = 20L
`macho_file_set` = 21L
`minidump` = 22L
`coff_cl_gl_object` = 23L
`coff_object` = 24L
`coff_import_library` = 25L
`pecoff_executable` = 26L
`windows_resource` = 27L
`xcoff_object_32` = 28L
`xcoff_object_64` = 29L
`wasm_object` = 30L
`pdb` = 31L
...
`cuda_fatbinary` = 33L
`offload_binary` = 34L
`dxcontainer_object` = 35L



 `Impl`  = structure(0:35, names = c("unknown", "bitcode", "archive", "elf", 
"elf_relocatable", "elf_executable", "elf_shared_object", "elf_core", 
"goff_object", "macho_object", "macho_executable", "macho_fixed_virtual_memory_shared_lib", 
...
"macho_file_set", "minidump", "coff_cl_gl_object", "coff_object", 
"coff_import_library", "pecoff_executable", "windows_resource", 
"xcoff_object_32", "xcoff_object_64", "wasm_object", "pdb", "tapi_file", 
"cuda_fatbinary", "offload_binary", "dxcontainer_object"))
...
DW_AT_APPLE_objc_direct = 297L, DW_AT_APPLE_sdk = 298L, DW_AT_lo_user = 8192L, 
DW_AT_hi_user = 16383L)
...
"FMax", "FMinimum", "FMaximum", "FMulAdd", "SelectICmp", "SelectFCmp"
))
...
`Unknown` = 1L
`Forward` = 2L
`ForwardButPreventsForwarding` = 3L
`Backward` = 4L
`BackwardVectorizable` = 5L
`BackwardVectorizableButPreventsForwarding` = 6L



 `DepType`  = structure(0:6, names = c("NoDep", "Unknown", "Forward", "ForwardButPreventsForwarding", 
